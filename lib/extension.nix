{
  ...
  pretty = value: (lib.generators.toPretty {} (
  ...
  toExtensionWithConfig = f:
    ...
    then
      config: final: prev: let
        fConfig = f prev;
      in
        if lib.isFunction fConfig
        then let
          fConfigPrev = f config prev;
        in
          if lib.isFunction fConfigPrev
          then
...
in {
  extension = {
    ...
    in {
      ...
      modules =
        if lib.isFunction modules
        then modules fixedConfig
        else modules;

      config = fixedConfig;
      __unfixConfig__ = config;
      withConfig = configOverlay: f (lib.composeExtensions config configOverlay);
      withFixedConfig = f;
    };
  in
    f config;
  ...
  extensions = let
    allExtensions = map delib.callExtension (delib.umport {paths = [./extensions];});
  ...
  in
    ...
    then delib.extension {inherit name;}
    else if totalExtensions == 1
    then builtins.removeAttrs (builtins.elemAt extensions 0) ["_file"]
    else let
      config = let
        sorted = builtins.sort (q: p: q.configOrder < p.configOrder) extensions;
        configs = builtins.map (x: x.__unfixConfig__) sorted;
      in
        lib.composeManyExtensions configs;
    in
      delib.extension {
        inherit name;
        description = let
          ...
          withUniqueDescription =
            lib.foldl' (
              acc: e:
                if builtins.any (extension: e.description == extension.description) acc
                then acc
                else acc ++ [e]
            ) []
            withDescription;
        ...
        in
          if totalUnique == 0
          then null
          else if totalUnique == 1
          then (builtins.head withDescription).description
          else
            lib.warn (
        ...
        in
          ...
          then (builtins.head withInitialConfig).initialConfig
          else
            lib.warn (
        ...
        libExtension = let
          ...
          libExtensions = config: builtins.map (extension: (extension.withFixedConfig config).libExtension) sorted;
        in
          config: lib.composeManyExtensions (libExtensions config);
        modules = config: builtins.concatMap (extension: (extension.withFixedConfig config).modules) extensions;
      };
}
