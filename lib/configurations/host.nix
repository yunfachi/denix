...
}:
{
        ...
        (
          ...
          in
          {
            ...
            imports =
              ...
              ++ (
                if currentHostName == name then
                  apply.listOfEverything (wrap myconfig) (wrap nixos) (wrap home) (wrap darwin)
                else
                  [ ]
              );
          }
  ...
  hostSubmoduleOptions = with delib.options; {
    ...
    rice = allowNull (enumOption (builtins.attrNames (config.${myconfigName}.rices or { })) null);
  };

  hostOption = host: with delib.options; noDefault (submoduleOption host null);
  hostsOption = host: with delib.options; attrsOfOption (submodule host) { };
